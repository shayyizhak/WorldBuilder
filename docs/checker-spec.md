...
### 2.5 The accounting property — both halves, or neither

Two invariants, and they must be stated and tested together. Each is trivially satisfiable on its own, and satisfying one while breaking the other is exactly what happened in rounds 12 and 13.
...
**ACCOUNTING** says nothing is silently dropped *after* extraction. Round 12 violated it: `partition-sum` extracted 33 assertions and checked 1, with the other 32 vanishing into an early return.

**FLOOR** says nothing is silently dropped *before* extraction. Round 13 violated it while satisfying ACCOUNTING perfectly: extraction collapsed to 2 assertions, all of them checked, `unresolvable` zero, `accounted: true` everywhere — against prose that had not changed a word.

Neither invariant alone is worth anything:

- ACCOUNTING alone is satisfiable by extracting nothing.
- FLOOR alone is satisfiable by extracting everything and checking nothing.

Treat them as a single named property — call it `coverage-sound` — asserted per rule per scope on every run. A build where either half fails is a failing build, regardless of how many findings the checker produced.

The history is worth keeping in mind when changing extraction or resolution code. Round 11: rules never received input. Round 12: rules received input and discarded it. Round 13: rules stopped receiving input again. The same defect class three times, in three different pieces of plumbing, and each fix created the next. This is not a badly built checker — it is a mechanism with several silent paths and, until now, no test asserting any of them stayed open.
