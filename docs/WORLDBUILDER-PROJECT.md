...
**The holdout rate is a draw from an uncharacterised distribution, and is retired as a halt condition.** It was used as one — *"the panel rate is 36% against ruleset 5's 33%, the halt condition on an unlike rate is clear"* — and the ruleset 7 → 8 cut is the natural experiment that voids it: **34.5% → 44.8% on worlds that are byte-identical apart from fourteen payload keys nothing reads.** Same model, same digest, all 27 state components equal. The rate moved ten points because a cold cut has no render cache to inherit, so a non-deterministic model rewrote every section and a different half of the chronicle fell out of canon. A ruleset bump changes worlds, so *every* cross-ruleset cut is cold — which makes 36% / 34.5% / 44.8% three independent draws rather than a series, and the gate they cleared would have cleared or failed at roughly random. The re-cut of ruleset 8 against its own commit is the control: warm, cache-hit, byte-identical prose, zero movement. **Compare the rate across warm cuts; never across cold ones, and never as a gate.** What survives untouched is everything `wb holdouts` says about code structure — call sites, floors, which rules carry instrumentation — because none of that was ever a rate comparison. A third consequence is worth stating because it touches a settled principle: *cached renders are canon*, so canon is itself a draw, and a second cut of the same world would have held out a different half of it.
...
*Machine work — all three are done. Full report: `docs/archive/phase-preverification-machine-report.md`.*
